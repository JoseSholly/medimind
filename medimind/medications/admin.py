...
class PrescriptionDrugInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('drug_id', 'drug_name', 'dosage_instruction', 'frequency_per_day', 'duration_days', 'get_status')
...
class PrescriptionDrugAdmin(admin.ModelAdmin):
    ...
    def generate_schedule_default(self, request, queryset):
        ...
        self.message_user(request, _("Successfully generated default schedules for selected prescription drugs."))
    generate_schedule_default.short_description = _("Generate default schedule (8 AM baseline)")

    def generate_schedule_strict(self, request, queryset):
        for drug in queryset:
            drug.generate_schedule(strict=True)
        self.message_user(request, _("Successfully generated strict schedules for selected prescription drugs."))
...
class PrescriptionLogAdmin(admin.ModelAdmin):
    ...
    list_filter = ('taken', 'date', 'prescription_drug__prescription__doctor', 'prescription_drug__prescription__patient')
    search_fields = (
        'log_id',
